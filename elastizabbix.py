...
hostname = sys.argv[1]
stats = {
    'cluster': "http://"+hostname+":9200/_cluster/stats",
    'nodes'  : "http://"+hostname+":9200/_nodes/stats",
    'indices': "http://"+hostname+":9200/_stats",
    'health' : "http://"+hostname+":9200/_cluster/health"
}
...
def get_cache(api):
    cache = '/tmp/elastizabbix-{0}_{1}.json'.format(api, hostname)
    lock = '/tmp/elastizabbix-{0}_{1}.lock'.format(api, hostname)
